...
<div class="card">
	...
	<div class="card-body">
	  <div class="table-responsive">
		<table class="table">
		  ...
		  <tbody>
				@foreach($bophan as $cv)
			<tr>
			  <th scope="row">{{ $cv->id }}</th>
			  <td>{{ $cv->ten_bo_phan }}</td>
			  <td>
	...
	<div class="card-header">
		<form action="{{url('tao-moi-bo-phan')}}" method="POST">
			{{ csrf_field() }}
			<div class="form-group row">
				<label for="inputEmail3" class="col-sm-3 col-form-label">Tên bộ phận</label>
				<div class="col-sm-9">
					<input type="text" class="form-control" name="tenbophan" placeholder="Tên bộ phận">
				</div>
			</div>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form id="form-edit" action="{{url('sua-bo-phan')}}" method="POST">
						{{ csrf_field() }}
						<input type="hidden" name="id" id="macv">
							<div class="form-group">
								<label for="recipient-name" class="form-control-label">Tên bộ phận</label>
								<input type="text" class="form-control" id="textedit" name="tenbophan">
							</div>
	...
	<div class="modal fade" id="xoaModal" tabindex="-1" role="dialog" aria-labelledby="exampleMLabel" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title" id="exampleMLabel">Xóa bộ phận ?!</h4>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<div class="modal-body">
						...
						<p>Nếu bạn xóa tên bộ phận này đồng nghĩa <Strong class="text-danger">tất cả dữ liệu liên quan đều bị xóa</Strong></p>
					</div>
					<div class="modal-footer">
							<form id="form-delete" action="{{url('xoa-bo-phan')}}" method="POST">
									{{ csrf_field() }}
